...
import org.tang.exam.rest.day.AddTaskWeeklyReq;
 ...
 *
 */
public class PostTaskWeeklyActivity extends BaseActionBarActivity  {
	private static final String TAG = "PostTaskWeeklyActivity";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_post_taskweekly);
		initView();
		
		selectDate = getIntent().getStringExtra("org.tang.exam.activity.PostTaskWeeklyActivity.selectDate");
		Log.d(TAG, "selectDate::::"+selectDate);
		ActionBar bar = getSupportActionBar();
		bar.setTitle(getResources().getString(R.string.task_weekly));
		bar.setDisplayHomeAsUpEnabled(true);
	}


	private void initView() {
		prgDialog = new ProgressDialog(PostTaskWeeklyActivity.this);
		etTitle = (EditText) findViewById(R.id.et_taskweekly_title);
		etContent = (EditText) findViewById(R.id.et_taskweekly_content);
	}
	...
	private void submitTaskWeekly(){
		UserInfo userInfo = UserCache.getInstance().getUserInfo();
		AddTaskWeeklyReq anotice = new AddTaskWeeklyReq();
		TaskWeekly notice = new TaskWeekly();
		...
		notice.setCreateDate(selectDate);
		anotice.setTaskWeekly(notice);
						...
						try {
							...
							if(rootObj.getString("sessionKey")!=null && !rootObj.getString("sessionKey").equals("")){
								if(AppConstant.notice_add_success==rootObj.getInt("msgFlag")){
									MessageBox.showMessage(PostTaskWeeklyActivity.this, "公告发布成功");
								}
								else{
									MessageBox.showMessage(PostTaskWeeklyActivity.this, "服务器异常");
								}
